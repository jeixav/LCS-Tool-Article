...
\end{gather*}
Periodic boundary conditions are imposed in the $x$-direction.The forcing function we choose is a chaotic solution of the Duffing oscillators with parameters in the chaotic regime, specifically
\begin{gather*}
\frac{d \phi_1}{dt} = \phi_2,\\
\frac{d \phi_1}{dt} = -0.1 \phi_2 - \phi_1^3 + 11 \cos(t),\\
f_{1,2}(t) = 2.625 \times 10^{-2} \phi_1(t/6.238 \times 10^5)
\end{gather*}
The parameter values used are: $U = 62.66$, $c_2 = 0.205 U$, $c_3 = 0.461 U$, $L_y = 1.77 \times 10^6$, $\epsilon_1 = 0.0075$, $\epsilon_2 = 0.04$, $\epsilon_3 = 0.3$, $L_x = 6.371 \times 10^6 \pi$, $k_n = 2 n \pi/L_x$, $\sigma_1 = 0.5 k_2 (c_2 - c_3)$, $\sigma_2 = 2 \sigma_1$. The domain of the flow is set to $x \in [0,L_x]$, $y \in [-2.25 L_y, 2.25 L_y]$ and the timespan is $t \in [0,4 L_x/U]$. We skip listing the file that defines the velocity equations; in regards to input and output variables, it is similar to the double gyre function (Listing~\ref{l:double gyre derivative}.) The LCS Tool script that specifies the parameters we are using is given in Listing~\ref{l:Bickley jet input}. Note the use of the \texttt{periodicBc} variable to specify periodic boundary conditions.
