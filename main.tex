% FIXME Add 5p option to get JCompSci two column layout
...
% FIXME Some figures use PDF version 1.6, PDFLaTeX default is 1.5
...
% FIXME Confirm George can be corresponding author
\author{G.~Haller}
...
\begin{keyword}
% FIXME Match keywords and PACS and MSC classifications
% FIXME Consult previous publications to find keywords
Lagrangian coherent structures \sep Mathematical software \sep Dynamical systems \sep Invariant manifolds \sep Mixing

% FIXME Verify these classifications, http://www.aip.org/pacs
\PACS 47.10.Fg \sep 47.27.De \sep 47.11.-j

% FIXME Verify these classifications http://www.ams.org/msc/msc2010.html
...
% FIXME JCompSci editor: Replace GitHub link with http://runmycode.org/
...
% FIXME Reviewer #3, comment #3: The comparison with other publicly available LCS detection software should include more details. It contains a very high-level summary of LCS tool being an "all-purpose numerical engine". That part should be revisited and become more concrete.
...
% FIXME Cannot cite LCS MATLAB Kit without a journal reference
% \item \emph{LCS MATLAB Kit}\citep{dabiri09:_lmk} calculates the FTLE from velocity datasets. Includes a graphical user interface.
...
% FIXME Cannot cite FlowVC without a journal reference
% \item \emph{FlowVC}\citep{shadden10:_flowvc} is a general purpose LCS platform for two and three dimensional datasets. Parallel calculations are supported with Open\-MP, CUDA and OpenCL.
% FIXME FlowTK
% https://github.com/FlowPhysics/FlowTK
% doi:10.1007%2F978-3-319-04099-8_13
\end{itemize}
...
% FIXME Reviewer #3, comment #1: Between the three applications, there should be a description about linking them in terms of generic APIs (application programming interface) or common code style. It is good to have code in the paper but it doesn't seem straightforward for a reader to find out how this simplifies the effort from coding from scratch (instead of using the platform). That part should be highlighted as a key point for judging the merit to the community. So, it would be very useful to pinpoint the generics with a flowchart (e.g. it always starts from input parameters, velocity definition and then LCS parameters, and how you finally call only 1-2 functions for the core solving routine), and your Matlab code there can further demonstrate the paradigm.
% FIXME Reviewer #5, comment #6: The functionality and taxonomy of the proposed LCS tool should be introduced for an overview understanding of the tool.
...
The Cartesian grid of initial conditions mentioned in \cref{t:Cauchy-Green algorithm} is rectangular, with user-defined vertical and horizontal ranges and resolutions. The optimal resolution may be determined by a successive doubling of the initial resolution until convergence of the extracted LCSs is observed visually.
% FIXME Reviewer #2, comment #1: "500 grid point" → "500 grid points"
If the domain of interest comprises only a few expected LCSs, e.g., one vortex, then a resolution of about 500 grid point along the longest axis usually gives good results. Otherwise, a higher resolution must be chosen.
...
\end{center}
% FIXME Reviewer #2, comment #5: Indicate what are "s" and "P(s)"
...
% FIXME Reviewer #3, comment #2: "As for a platform, efficiency is also a matter in consideration. Performance degradation is not expected as a cost of generalized computational paradigm. That part should be at least mentioned along with the applications to make the article technically more sound."
...
\lstinputlisting[caption={LCS Tool commands for double gyre elliptic LCSs. Subset of the LCS Tool script \lstinline!demo/double_gyre/hyperbolic_shear_lcs.m!},
                 label=l:double gyre lambda-line LCS]{listings/double_gyre/elliptic_lcs_subset.m}
...
% FIXME Reviewer #2, comment #6: Explain meaning of white dots ($\lambda$ maxima) and, if possible improve their appearance
\caption{LCSs in the double gyre flow. Resolution $[500\times250]$. $\lambda = 1.00$ and $1.04$ for the left and the right gyre. $\lambda \in [0.93,1.07]$, $\Delta\lambda = 0.01$.
(Top) Elliptic (green) and repelling (red) LCSs with the FTLE shown in the background. (Bottom) Elliptic and attracting (blue) LCSs.}
...
\caption{Elliptic and hyperbolic (top: red - repelling, bottom: blue - attracting) LCSs in the Bickley jet. $\lambda$ values of elliptic LCSs from left to right are [0.95,\,0.80,\,0.94,\,0.80,\,0.94], $\lambda \in [0.80,1.20]$, $\Delta\lambda = 0.01$.}
...
% FIXME Keywords are required to be in British English (http://www.elsevier.com/journals/journal-of-computational-science/1877-7503/guide-for-authors), so should switch entire document to British English
% Local Variables:
% ispell-dictionary: "en_US"
% End:
% LocalWords:  stretchline stretchlines incompressible gyre eigenvector eigenvectors incompressibility dataset datasets LCS LCSs Lyapunov Bickley Rossby timespan variational adaptively vortices parametrise advected strainlines minima extremal FTLE MATLAB maxima geostrophic shrinkline shrinklines
